...
Arguments:
    arg_1: The  environment  for  the  workspace for the status
    arg_2: The name of the workspace for the status
...
result=$(${CDP_PATH}/cdp ml describe-workspace --environment-name $env_name --workspace-name $ws_name  | jq -r '.workspace."instanceStatus"')
handle_exception $? "$prefix" "ml workspace list" "$result"
echo "┃ Workspace $ws_name in environment $env_name is $result  ┃"
