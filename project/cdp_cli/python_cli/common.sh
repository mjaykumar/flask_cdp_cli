env_variables()
{
        CDP_PATH=/home/centos/cdpclienv/bin
        BASE_SCRIPT_DIR=/home/centos/project/cli/iqvia/python_cli
}
...
#####################
# Arguments:        #
#   $1 -> complete name of custom log file #
#   $2 -> username    #
#####################
...
enable_logger()
{
        logfileName=$1
        username=$2
        sudo cp /etc/rsyslog.conf ~/
        sudo chown  $username:$username ~/rsyslog.conf
        echo 'local7.*   -$logfileName' >> ~/rsyslog.conf
        sudo cp ~/rsyslog.conf /etc/
        sudo service rsyslog.service restart
